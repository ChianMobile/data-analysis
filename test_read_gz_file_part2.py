...
def get_file_line(paths):
    ...
    try:
        for path in paths:
            ...
            gzfile = file(os.path.join(r'G:\MobileData', outputfile_wan), 'a')
            gzfile.write('timestamp,deviceid,wanindex,wanname,wanavertxrate,wanaverrxrate,wanmaxtxrate,wanmaxrxrate\r')
            subdevice = file(os.path.join(r'G:\MobileData', outputfile_lan), 'a')
            subdevice.write('deviceid,subdevicenumber,subdevicename,subdevicemac,subdeviceavertxrate,subdeviceaverrxrate,lanupstatistics,landownstatistics\r')
                    ...
                    if sa[21] != '':
                        ...
                        if sa[index_a] == '1':
                            if len(sa) >= index_a + 11:
                                i = 0
                                lan = sa[4] + ',' + sa[index_a] + ',' + sa[int(index_a) + 2 + i * 11] + ',' + sa[int(index_a) + 5 + i * 11] +\
