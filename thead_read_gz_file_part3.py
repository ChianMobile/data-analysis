...
def get_file_line(path):
    ...
    try:
        ...
        gzfile = file(os.path.join(r'G:\MobileData\tFile', outputfile_wan), 'a')
        gzfile.write('timestamp,deviceid,wanindex,wanname,wanavertxrate,wanaverrxrate,wanmaxtxrate,wanmaxrxrate\r')
        subdevice = file(os.path.join(r'G:\MobileData\tFile', outputfile_lan), 'a')
        ...
        con = read_gz_file(os.path.join(r'G:\MobileData\chen2\HGUPERIODIC', path))
...
if __name__ == '__main__':
    a = time.clock()
    thread_list = []
    paths = get_gz_fileName(r'G:\MobileData\chen2\HGUPERIODIC')
    for path in paths:
